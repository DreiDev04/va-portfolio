...
function Pillar({
  ...
  return (
    ...
    >
      ...
      <h3 className="text-2xl md:text-3xl font-black text-foreground tracking-tight mb-4 group-hover:text-electric transition-colors">
        {label}
      </h3>
  ...
  return (
    <section id="pivot" className="bg-background border-t-2 border-border">
      <div className="max-w-screen-xl mx-auto border-l-2 border-r-2 border-border">
        ...
        {/* Bottom: pillar grid */}
        <div className="bg-border gap-px grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {PIVOT.pillars.map((p, i) => (
            <div key={p.number} className="bg-background">
              <Pillar
                number={p.number}
                label={p.label}
